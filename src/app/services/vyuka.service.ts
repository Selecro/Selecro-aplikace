...
export class VyukaService {
  ...
  vyuk: Map<string,Vyuka>;

  constructor() {
    this.initMap();
  }

  public getVideoByName(name: string): Vyuka{
    return this.vyuk.get(name);
  }

  private initMap(){
    this.vyuk = new Map();
    poleVyuk.forEach(vyuka=>{
      this.vyuk.set(vyuka.nazev, vyuka);
    });
  }
}
